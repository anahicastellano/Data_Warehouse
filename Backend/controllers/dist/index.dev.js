...
var db = new Sequelize(process.env.DB_NAME, process.env.DB_USERNAME, 'D4T4IS@W3S0m3!i5N\'T1t?', {
  host: 'localhost',
  port: 3306,
  dialect: 'mysql'
});

function findOne(query, id, type) {
  var results;
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 2:
          ...
          throw new Error("No existe ".concat(type, " con id ").concat(id));
...
function getResourceById(table, id) {
  return findOne("select * from ".concat(table, " where id = :id"), id, table);
}
